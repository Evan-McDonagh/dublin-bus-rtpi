...
def init(request):
    ...
    if request.method == 'POST':
        ...
        address_request = requests.get(
            'https://maps.googleapis.com/maps/api/geocode/json?latlng={},{}&key={}'.format(lat, lng, gmap_api))
                    ...
                    for stopkey in allstops:
                        if stop == stopkey:
                            ...
                            if isInbounds(bounds, STOP):
                                alongroutestopinfos.append(
                                    {"id": STOP['stopno'], 'lat': STOP["latitude"], 'lng': STOP["longitude"]})
...
# to match all stops to see if the stop is standing in the route.
...
def rtmarkerinfo(request):
    if request.method == 'POST':
        ...
        for result in obj_json['results']:
            ...
            rsp[obj_json.get('stopid')].append(
                {key: {'arrivaltime': result.get('arrivaldatetime'), 'destination': result.get('destination')}})
            allinfo += "Route:" + key + "  arrive at:" + result.get('arrivaldatetime') + " Towards " + result.get(
                'destination') + "<br>"
