...
def predict_travel_time(route, direction, datetime_departure, feels_like, weather_main, stopA, stopB):
    ...
    stopB_frac = stopfractions[stopfractions['LINEID'] == route][stopfractions['DIRECTION'] == direction][
        stopfractions['STOPPOINTID'] == stopB]['TRIP_FRAC'].iloc[0]
    fraction = stopB_frac - stopA_frac

    schedule = [
        datetime_departure.replace(hour=0, minute=0, second=0, microsecond=0) + datetime.timedelta(seconds=int(t)) for t
